...
helm_mock_args = \
	...
	$(if $(filter true,$(ENABLE_SERVICENOW_MOCK)),--set-string network.mcpSecrets.servicenow.username=admin,) \
	$(if $(filter true,$(ENABLE_SERVICENOW_MOCK)),--set-string network.mcpSecrets.servicenow.password=admin,) \
